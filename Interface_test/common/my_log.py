...
class MyLog:
    ...
    def my_log(level, msg):
        ...
        my_logger = logging.getLogger('api-log')  # 创建日志器对象
        my_logger.setLevel('DEBUG')  # 设置日志打印级别
        formatter = logging.Formatter('[%(asctime)s]-[%(levelname)s]-[日志信息]:%(message)s')  # 创建格式化器
